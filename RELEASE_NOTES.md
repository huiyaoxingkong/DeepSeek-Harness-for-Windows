...
### 0.2 启动服务器期间切换页签导致界面「异常放大」/卡死

**现象**：点「启动服务器」后（首次构建核心可能十几分钟，日常启动也要 1–3 分钟），在等待
期间点击左侧页签（设置 / 插件…），服务器就绪时界面突然变成满屏：内容被拉满、页头消失、
侧边栏不见，而且**没有任何返回入口**（连「退出全屏」按钮也看不到）。

**根因**：`openFrame`（服务器启动完成后调用）无条件执行 `setImmersive(true)` 进入沉浸全屏。
沉浸模式的 CSS 会隐藏侧边栏与页头、把 `.content` 的 padding 归零并禁止滚动、把 `.page`
撑成 `100vh`；而「退出全屏」按钮位于**工作台页内部**，此时工作台页已因用户切换页签而
`display:none`，于是既没有导航也没有退出入口——只能重启应用。

**修复**：只有用户**仍停留在工作台页**时才自动进入沉浸全屏；离开工作台页（`showPage`）
会自动退出全屏，恢复侧边栏、页头与滚动。

| 观测项 | 修复前 | 修复后 |
| --- | --- | --- |
| `body` class | `immersive` | 无（正常布局） |
| 侧边栏 | `display: none`（隐藏） | `flex` |
| `.content` padding / overflow | `0px` / `hidden`（满屏、不可滚） | `22px 26px` / `auto` |
| 页头 | 隐藏 | 显示 |
| 当前页签标记 | 丢失（`navActive` 为空） | 停在用户点击的页签（`settings`） |
| iframe | 已加载（但在隐藏页里） | 已加载，回到工作台即可见 |

### 0.3 关闭应用：先确认、保存设置、关闭内核服务器

**改进**：点窗口 X 不再直接退出，而是弹出确认对话框（取消 / 最小化到托盘 / 关闭应用）。

- **关闭应用**：按顺序执行「保存设置（写入 `config.json`）→ 停止内核服务器 → 停止托盘图标
  → 销毁窗口」，退出后不会留下孤儿 node 进程；`webview.start()` 的 `finally` 还会再兜底保存
  一次并停止内核，任何退出路径（含托盘退出、升级前退出）都满足同一保证。
- **最小化到托盘**：窗口隐藏，内核服务器继续运行（托盘菜单可再打开）。
- **取消**：继续使用。
- **设置开关**：新增「关闭窗口时先弹出确认界面」（`close_confirm`，默认开启）；关闭后恢复旧
  行为（按「关闭窗口时最小化到系统托盘」决定）。
- **不会被困住**：确认界面出现后再按一次窗口 X 视为确认并直接退出，外壳界面无响应时也能关闭。

| 观测项 | 结果 |
| --- | --- |
| 收到关闭请求（`poll_tray` 返回 `close: true`） | 弹出对话框（三个动作齐全，实测） |
| 点「取消」 | 对话框关闭且调用 `cancel_close`，应用继续运行 |
...
Windows 上「杀掉父进程」不会杀掉它生成的子进程：`pnpm.cmd`（cmd.exe）被终止后，它启动的
node 仍持有管道的写端。凡是「管道捕获 + 超时」的写法都会在超时后**永久阻塞**——本轮排查出
三处并全部修好：

| 位置 | 原风险 | 修复 |
| --- | --- | --- |
| profile store 自愈的 pnpm install（启动后台线程） | `subprocess.run(capture_output=True, timeout=1800)`：超时后 `communicate()` 永久阻塞 → `_heal_done` 永不置位 → **所有插件操作永久卡在「依赖修复尚未完成」** | 输出写 `data\store-heal-pnpm.log`，超时用 `taskkill /F /T` 杀进程树，调用方一定拿到结果 |
| 健康检查 `dsh --dump-config` | 同样形态（内核还会再生成插件子进程） | 改用同一套文件输出 + 杀树 |
| 核心更新的 pnpm install / build | `for line in proc.stdout` 读原始管道且**完全不响应取消**：点「取消更新」后仍要等整轮 pnpm 跑完（数分钟到数十分钟） | 改为守护线程读输出 + `stop=self._cancel`，取消即刻杀进程树并返回「更新已取消」 |

真实进程实测（`homes.run_capture` / `run_stream`）：挂死的 node 在超时后 3.7s 内被杀并返回；
运行中取消在 2.2s 内杀树返回，且已产生的输出仍然收到。
...
| 测试 | 内容 | 结果 |
| --- | --- | --- |
| `tools/test-1.0.5.py` | **259 项**：超 MAX_PATH 删除、只读文件、junction 不跟随、入口解析 5 种布局、启动梯度、内核打印地址（token/裸地址/无输出）、usage 错误识别、控制台输出解码、健康检查容忍度、换核 / 回滚 / 陈旧备份、外壳 UI 同步（含不降级）、CSS 不变式、**重复 id / 主题加载 / i18n 空白匹配 / 取消更新控件 / 示例插件不随包 / 卸载校验 / 旧插件迁移（manifest + pnpm 对账）/ 预设包集合 / 随包商店包版本与再打包默认值** | **259/259 通过** |
